...
('mensagens-imagens', 'mensagens-imagens', true),
('mensagens-videos', 'mensagens-videos', true),
('mensagens-audios', 'mensagens-audios', true),
('mensagens-documentos', 'mensagens-documentos', true);

-- Políticas de segurança para os buckets
CREATE POLICY "Acesso público aos avatares" ON storage.objects
FOR SELECT USING (bucket_id = 'avatars');

CREATE POLICY "Usuários autenticados podem fazer upload de avatares" ON storage.objects
FOR INSERT WITH CHECK (
  bucket_id = 'avatars' 
  AND auth.role() = 'authenticated'
);

CREATE POLICY "Usuários podem gerenciar seus próprios avatares" ON storage.objects
FOR ALL USING (
  bucket_id = 'avatars' 
  AND auth.uid()::text = (storage.foldername(name))[1]
);

-- Políticas para mensagens de mídia
CREATE POLICY "Acesso a mídias de mensagens" ON storage.objects
FOR SELECT USING (
  bucket_id IN ('mensagens-imagens', 'mensagens-videos', 'mensagens-audios', 'mensagens-documentos')
);

CREATE POLICY "Upload de mídias por usuários autenticados" ON storage.objects
FOR INSERT WITH CHECK (
  bucket_id IN ('mensagens-imagens', 'mensagens-videos', 'mensagens-audios', 'mensagens-documentos')
  AND auth.role() = 'authenticated'
);
